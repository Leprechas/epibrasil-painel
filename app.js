const MANIFEST_URL="data/manifest.json?v=20260512-6";
const GEO_UF_URL="data/ufs.geojson?v=20260512-6";
const GEO_MUN_URL="data/municipios.geojson?v=20260512-6";
const $=id=>document.getElementById(id);
const fmt=new Intl.NumberFormat("pt-BR");
const fmt1=new Intl.NumberFormat("pt-BR",{maximumFractionDigits:1});
let MANIFEST=[],DATA=[],GEO_UF=null,GEO_MUN=null,MAP_UF=null,MAP_MUN=null,LAYER_UF=null,LAYER_MUN=null;
const CACHE={};

function selectedYears(){
  return [...$("year").selectedOptions]
...
function setSelectedYears(years){
  ...
  [...$("year").options].forEach(o=>{
    o.selected=ys.has(Number(o.value));
  });
}
...
function norm(r){const casos=num(r.casos)||0,pop=num(r.populacao);let inc=num(r.incidencia_100mil);if((inc===null||!Number.isFinite(inc))&&pop>0)inc=casos/pop*100000;return{doenca:(r.doenca||"").toUpperCase(),doenca_nome:r.doenca_nome||r.doenca,cod_mun6:String(r.cod_mun6||"").padStart(6,"0").slice(0,6),municipio:r.municipio||"",uf:(r.uf||"").toUpperCase(),ano:Number(r.ano),casos,populacao:pop,incidencia_100mil:inc}}
...
async function loadDisease(code){if(CACHE[code]){DATA=CACHE[code];return}const item=MANIFEST.find(d=>d.codigo===code);if(!item)throw new Error("Doença não encontrada");$("row-count").textContent="carregando "+item.doenca+"...";const res=await fetch(item.arquivo+"?v=20260512-6");if(!res.ok)throw new Error("Arquivo da doença não encontrado: "+item.arquivo);DATA=parseCSV(await res.text()).map(norm).filter(r=>r.doenca&&r.cod_mun6&&Number.isFinite(r.ano));CACHE[code]=DATA;$("row-count").textContent=`${fmt.format(DATA.length)} linhas nesta doença | ${fmt.format(MANIFEST.reduce((a,b)=>a+b.linhas,0))} linhas no total`}
...
function setupFilters(){
  const oldYears=selectedYears(),oldUf=$("uf").value;
  ...
  if(validOld.length>0){
    setSelectedYears(validOld);
  }else{
    setSelectedYears([years.at(-1)]);
  }
...
function updateMuns(){
  ...
  const muns=[...new Set(
    DATA
      .filter(r=>(years.length===0||years.includes(r.ano))&&(!uf||r.uf===uf))
      .map(r=>r.municipio)
      .filter(Boolean)
...
function rowsForSelectedYearsNoGeoFilters(){
  const years=selectedYears();
  return DATA.filter(r=>years.length===0||years.includes(r.ano));
}
...
const UF_BY_PREFIX={"11":"RO","12":"AC","13":"AM","14":"RR","15":"PA","16":"AP","17":"TO","21":"MA","22":"PI","23":"CE","24":"RN","25":"PB","26":"PE","27":"AL","28":"SE","29":"BA","31":"MG","32":"ES","33":"RJ","35":"SP","41":"PR","42":"SC","43":"RS","50":"MS","51":"MT","52":"GO","53":"DF"};
...
function initMaps(){if(!MAP_UF){MAP_UF=L.map("map-uf",{scrollWheelZoom:false}).setView([-14.2,-51.9],4);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap"}).addTo(MAP_UF)}if(!MAP_MUN){MAP_MUN=L.map("map-mun",{scrollWheelZoom:false}).setView([-14.2,-51.9],4);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap"}).addTo(MAP_MUN)}}
...
const ser=group(rows,["ano"]),["ano"]).sort((a,b)=>a.ano-b.ano);Plotly.newPlot("series",[{x:ser.map(r=>r.ano),y:ser.map(r=>r.casos),type:"bar",name:"Casos",marker:{color:"#1f77b4"}}],{margin:{t:20,r:20,b:45,l:60},yaxis:{title:"Casos/Ano"},xaxis:{title:"Ano"}},{responsive:true,displayModeBar:false});
const ind=$("indicator").value;const ufRank=group(rowsForSelectedYearsNoGeoFilters(),["uf"]).filter(r=>r.uf&&(r[ind]||0)>0).sort((a,b)=>(b[ind]??-Infinity)-(a[ind]??-Infinity));Plotly.newPlot("ufs",[{x:ufRank.map(r=>r[ind]),y:ufRank.map(r=>r.uf),type:"bar",orientation:"h",marker:{color:ufRank.map((_,i)=>i<5?"#1e3a8a":"#60a5fa")},text:ufRank.map(r=>fmt.format(r[ind]||0)),textposition:"inside",insidetextanchor:"end",hovertemplate:"<b>%{y}</b><br>Valor: %{x}<extra></extra>"}],{margin:{t:20,r:30,b:50,l:60},height:520,xaxis:{title:ind==="casos"?"Casos/Ano":"Incidência / 100 mil"},yaxis:{automargin:true,categoryorder:"total ascending"},plot_bgcolor:"#fff",paper_bgcolor:"#fff"},{responsive:true,displayModeBar:false});
const munRank=group(rows,["uf","cod_mun6","municipio"]).filter(r=>(r[ind]||0)>0).sort((a,b)=>(b[ind]??-Infinity)-(a[ind]??-Infinity)).slice(0,100);$("table").innerHTML="<thead><tr><th>UF</th><th>Código</th><th>Município</th><th>Casos/Ano</th><th>População/Ano</th><th>Incidência</th></tr></thead><tbody>"+munRank.map(r=>`<tr><td>${r.uf}</td><td>${r.cod_mun6}</td><td>${r.municipio}</td><td>${fmt.format(r.casos||0)}</td><td>${r.populacao?fmt.format(r.populacao):"—"}</td><td>${Number.isFinite(r.incidencia_100mil)?fmt1.format(r.incidencia_100mil):"—"}</td></tr>`).join("")+"</tbody>";renderMaps(rows)}
async function diseaseChanged(){await loadDisease($("disease").value);setupFilters();refresh()}
async function init(){const [manRes,ufRes,munRes]=await Promise.all([fetch(MANIFEST_URL),fetch(GEO_UF_URL).catch(()=>null),fetch(GEO_MUN_URL).catch(()=>null)]);if(!manRes.ok)throw new Error("Manifesto de dados não encontrado");MANIFEST=await manRes.json();if(ufRes&&ufRes.ok)GEO_UF=await ufRes.json();if(munRes&&munRes.ok)GEO_MUN=await munRes.json();setupDiseaseSelect();await diseaseChanged()}
